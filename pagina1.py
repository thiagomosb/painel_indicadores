...
def app():
    ...
    with tab1:
        ...
        if not df_escala_filtrado.empty or not df_turnos_filtrado.empty:
            ...
            # Lógica de preparação de dados (mantida igual)
            ...
            if not df_contrato_filtrado.empty and 'ESCALA' in df_contrato_filtrado.columns:
                ...
                for d in date_range:
                    count = total_contratos_30 if d.dayofweek == 6 else total_contratos_30 + total_contratos_24
            ...
            df_comparativo_diario_t2['Data'] = pd.to_datetime(df_comparativo_diario_t2['Data']).dt.strftime('%Y-%m-%d')
            
            # Garante que os dados estejam ordenados por data antes de criar o gráfico
            df_comparativo_diario_t2.sort_values('Data', inplace=True)


            # Conversão para Plotly
            fig_diario = px.bar(
                df_comparativo_diario_t2,
                x='Data',
                y=['Turnos Contratados', 'Escala sgd', 'Turnos Abertos NewMars'],
                barmode='group',
                text_auto=True,
                labels={'Data': '', 'value': '', 'variable': 'Fonte'},
                template='plotly_white',
                color_discrete_map={
                    'Turnos Contratados': '#36b9cc',
                    'Escala sgd': '#4e73df',
                    'Turnos Abertos NewMars': '#1cc88a'
                }
            )
            
            fig_diario.update_traces(textfont=dict(color='black', size=14), hovertemplate='%{data.name}: <b>%{y}</b><extra></extra>')
            
            # --- ALTERAÇÃO AQUI ---
            # Define o range inicial do eixo X para mostrar os últimos 7 dias
            initial_range = None
            if len(df_comparativo_diario_t2) > 10:
                # Pega a data de início (7 dias atrás) e a data final (último dia)
                start_date = df_comparativo_diario_t2['Data'].iloc[-12]
                end_date = df_comparativo_diario_t2['Data'].iloc[-1]
            ...
            fig_diario.update_layout(
                ...
                xaxis=dict(
                    ...
                    range=initial_range  # Define o range inicial do eixo X
                ),
            ...
            # Conversão para Plotly
            fig_regional = px.bar(
                df_comparativo_nova_regional,
                x='Regional',
                y=['Turnos Contratados', 'Escala sgd', 'Turnos Abertos NewMars'],
                barmode='group',
                text_auto=True,
                labels={'Regional': '', 'value': '', 'variable': 'Fonte'},
                template='plotly_white',
                color_discrete_map={
                    'Turnos Contratados': '#36b9cc',
                    'Escala sgd': '#4e73df',
            ...
            fig_regional.update_traces(textfont=dict(color='black', size=14), hovertemplate='%{data.name}: <b>%{y}</b><extra></extra>')
            ...
            # Conversão para Plotly
            fig_unidade = px.bar(
                df_comparativo_unidade,
                x='unidade',
                y=['Turnos Contratados', 'Escala sgd', 'Turnos Abertos NewMars'],
                barmode='group',
                text_auto=True,
                labels={'unidade': '', 'value': '', 'variable': 'Fonte'},
                template='plotly_white',
                color_discrete_map={
                    'Turnos Contratados': '#36b9cc',
                    'Escala sgd': '#4e73df',
            ...
            fig_unidade.update_traces(textfont=dict(color='black', size=14), hovertemplate='%{data.name}: <b>%{y}</b><extra></extra>')
            fig_unidade.update_layout(
                legend_title_text='',
                xaxis=dict(tickfont=dict(color='black', size=12)),
                xaxis_tickangle=-30, # Rotaciona os labels do eixo X
                yaxis=dict(showticklabels=False),
                hoverlabel=dict(bgcolor="white", font_size=16, font_color="black")
            )
            ...
            if 'cidade' in df_escala_filtrado.columns and 'cidade' in df_turnos_filtrado.columns:
                ...
                # Conversão para Plotly
                fig_cidade = px.bar(
                    df_comparativo_cidade,
                    x='cidade',
                    y=['Escala sgd', 'Turnos Abertos NewMars'],
                    barmode='group',
                    text_auto=True,
                    labels={'cidade': '', 'value': '', 'variable': 'Fonte'},
                    template='plotly_white',
                    color_discrete_map={
                        'Escala sgd': '#4e73df',
                        'Turnos Abertos NewMars': '#1cc88a'
                ...
                fig_cidade.update_traces(textfont=dict(color='black', size=14), hovertemplate='%{data.name}: <b>%{y}</b><extra></extra>')
