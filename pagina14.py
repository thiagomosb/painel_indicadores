    ...
    with INSPECAO_ESP:
        ...
        with col1:
            ...
            if not blitz_por_instrutor_sesmt_sem_pessoas.empty:
                ...
                fig_sesmt.update_layout(xaxis_title="", yaxis_title="", template="plotly_white",
                                        font=dict(color='black', size=14, family='Arial'),
                                        xaxis=dict(tickfont=dict(color='black', size=14)),
                                        yaxis=dict(tickfont=dict(color='black', size=14), range=[0, max(1, max_valor * 1.2)]),
                                        margin=dict(t=100))
        ...
        with col2:
            ...
            if not blitz_por_instrutor_coordenacao.empty:
                ...
                fig_coordenacao.update_layout(xaxis_title="", yaxis_title="", template="plotly_white",
                                            font=dict(color='black', size=14, family='Arial'),
                                            xaxis=dict(tickfont=dict(color='black', size=14)),
                                            yaxis=dict(tickfont=dict(color='black', size=14), range=[0, max(1, max_valor_coordenacao * 1.6)]),
                                            margin=dict(t=100))
        ...
        with col3:
            ...
            if not blitz_por_instrutor_supervisao.empty:
                ...
                fig_supervisao.update_layout(xaxis_title="", yaxis_title="", template="plotly_white",
                                            font=dict(color='black', size=14, family='Arial'),
                                            xaxis=dict(tickfont=dict(color='black', size=14)),
                                            yaxis=dict(tickfont=dict(color='black', size=14), range=[0, max(1, max_valor_supervisao * 1.6)]),
                                            margin=dict(t=100))
        ...
        with col4:
            ...
            if not blitz_por_instrutor_lider_campo.empty:
                ...
                fig_lider.update_layout(xaxis_title="", yaxis_title="", template="plotly_white",
                                        font=dict(color='black', size=14, family='Arial'),
                                        xaxis=dict(tickfont=dict(color='black', size=14)),
                                        yaxis=dict(tickfont=dict(color='black', size=14), range=[0, max(1, max_valor_lider * 1.6)]),
                                        margin=dict(t=100))
        ...
        with col5:    
            ...
            fig_totais.update_layout(
                ...
                yaxis=dict(tickfont=dict(color='black', size=14), range=[0, max(1, max_total * 1.25)]),
...
#
    ...
    with CONTATO:
        ...
        with col1:
            ...
            # Agrupar os dados
            inspecoes_por_equipe = df_filtrado.groupby("num_operacional", as_index=False).agg(
                quantidade_inspecoes=('idtb_turnos', 'nunique')
            )

            # Garantir que 'num_operacional' seja string
            inspecoes_por_equipe['num_operacional'] = inspecoes_por_equipe['num_operacional'].astype(str)
        ...
        with col2:
            ...
            inspecionadas_por_mes = df_filtrado.groupby(df_filtrado['data_turno'].dt.month)['num_operacional'].nunique()
            total_turnos_por_mes = df_turnos.groupby(df_turnos['dt_inicio'].dt.month)['num_operacional'].nunique()
        ...
        equipes_total_no_turno = sorted(df_turnos['num_operacional'].dropna().astype(str).unique())
        equipes_inspecionadas_no_turno = sorted(df_filtrado['num_operacional'].dropna().astype(str).unique())
        ...
        with colunas[0]:
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                df_eletricistas = df_filtrado[df_filtrado['funcao'].str.contains('Eletricista', case=False, na=False)][['nome', 'funcao', 'num_operacional']]
                df_eletricistas['Status Inspeção'] = df_eletricistas['num_operacional'].astype(str).apply(
                    lambda x: 'Inspecionada' if x in equipes_inspecionadas_no_turno else 'Não Inspecionada'
                )
                df_eletricistas = df_eletricistas.rename(columns={'nome': 'Nome', 'funcao': 'Função', 'num_operacional': 'Equipe'})
        ...
        for i, tipo in enumerate(tipos_equipe):
            with colunas[i + 1]:  # colunas[0] é o gráfico geral
                ...
                equipes_total_tipo = sorted(df_tipo_turnos['num_operacional'].dropna().astype(str).unique())
                equipes_inspecionadas_tipo = sorted(df_tipo_filtrado['num_operacional'].dropna().astype(str).unique())
    ...
    #------------------------------------------------ taxa de contato por tipo de prefixo ----------------------------------------------------#
        ...
        if not df_filtrado.empty:
            ...
            inspecionadas_prefixo = pd.merge(
                df_filtrado[['num_operacional', 'idtb_turnos']].drop_duplicates(),
                df_turnos[['idtb_turnos', 'descricao_tipo_prefixo']],
                on='idtb_turnos',
                how='left'
            ).groupby('descricao_tipo_prefixo')['num_operacional'].nunique()

            # Totais por tipo de prefixo (todas as equipes que abriram turno)
            total_prefixo = df_turnos.drop_duplicates(['num_operacional', 'idtb_turnos']).groupby('descricao_tipo_prefixo')['num_operacional'].nunique()
    ...
    with REPROVACAO:
        ...
        with col1:
                ...
                fig_subgrupo = go.Figure(data=[go.Pie(
                    labels=subgrupos_df['subgrupo'],
                    values=subgrupos_df['quantidade'],
                    hole=0.5,
                ...
                fig_subgrupo.update_layout(height=500,  # ⬅️ aumenta a altura para evitar corte
                    title={'text': ""},
                    annotations=[
                        dict(
    ...
    #----------------------------------------Não Conformidade Por Equipe -----------------------------------#
        ...
        with col1:
            ...
            # Mesclar os dois
            dados_equipes = pd.merge(inspecoes_por_equipe, reprovacoes_por_equipe, on='num_operacional', how='outer').fillna(0)
            ...
            fig.add_trace(go.Bar(
                x=dados_equipes['num_operacional'],
                y=dados_equipes['total_inspecoes'],
                name='Inspeções',
                marker_color='#1f77b4',
                text=dados_equipes['total_inspecoes'],
                textposition='outside',
                textfont=dict(size=16, color='black')  # Texto maior e preto
            ))
            ...
            # Layout ajustado
            fig.update_layout(height=550,  # ⬅️ aumenta a altura para evitar corte
                barmode='group',
                xaxis_title="",
                yaxis_title="",
                title="",
                ...
                hovermode='x unified',
                legend_title_text='',
                legend=dict(font=dict(size=16, color='black')),  # Legenda maior e preta
                xaxis=dict(type='category', tickfont=dict(size=14, color='black')),  # Eixo X preto
                yaxis=dict(tickfont=dict(size=14, color='black')),                   # Eixo Y preto
                font=dict(size=14, color='black')  # Texto geral
            )
    ...
    #----------------------------------------------------------------  
        ...
        eq_nc_comum_sesmt = sorted(list(set(eq_nc_sesmt) & set(eq_nc_lider)))
        eq_nc_comum_lider = sorted(list(set(eq_nc_lider) & set(eq_nc_sesmt)))

        equipes_ambos = sorted(set(eq_insp_sesmt) & set(eq_insp_lider))
        equipes_ambos_sem_nc = sorted([eq for eq in equipes_ambos if eq in eq_sem_nc_sesmt and eq in eq_sem_nc_lider])
        ...
        # Reprovações distintas (sem duplicar em comum)
        eq_nc_geral = sorted(set(eq_nc_sesmt) | set(eq_nc_lider))

        # Inspecionadas distintas
        eq_insp_geral = sorted(set(eq_insp_sesmt) | set(eq_insp_lider))

        # Sem reprovação (inspecionadas que não estão nas reprovadas)
        ...
        # Equipes em comum com NC (já temos)
        eq_nc_comum_geral = sorted(set(eq_nc_sesmt) & set(eq_nc_lider))
        ...
        df_equipes = pd.DataFrame({
            ...
            "EQUIPES (n)": [len(eq_insp_sesmt), len(eq_insp_lider)],
            "Equipes Inspecionadas": [resumir_com_tooltip(eq_insp_sesmt), resumir_com_tooltip(eq_insp_lider)],
            "REPROVAÇÕES (n)": [len(eq_nc_sesmt), len(eq_nc_lider)],
            "Equipes Reprovadas": [resumir_com_tooltip(eq_nc_sesmt), resumir_com_tooltip(eq_nc_lider)],
            "EM COMUM COM NC(n)": [len(eq_nc_comum_sesmt), len(eq_nc_comum_lider)],
            "Equipes em Comum NC": [resumir_com_tooltip(eq_nc_comum_sesmt), resumir_com_tooltip(eq_nc_comum_lider)],
            "SEM REPROVAÇÕES (n)": [len(eq_sem_nc_sesmt), len(eq_sem_nc_lider)],
            "Equipes Sem Reprovação": [resumir_com_tooltip(eq_sem_nc_sesmt), resumir_com_tooltip(eq_sem_nc_lider)],
            "EQUIPES NÃO INSPECIONADAS (n)": [len(eq_nao_insp_sesmt), len(eq_nao_insp_lider)],
            "Equipes Não Inspecionadas": [resumir_com_tooltip(eq_nao_insp_sesmt), resumir_com_tooltip(eq_nao_insp_lider)],
            "AMBOS SEM INSPEÇÃO (n)": [len(equipes_ambos_sem_inspecao)] * 2,
            "Equipes Ambos Sem Inspeção": [resumir_com_tooltip(equipes_ambos_sem_inspecao)] * 2
        })

        # =========================
        # Linha GERAL com total de reprovações distintas
        # =========================
        reprov_total_geral = len(eq_nc_sesmt) + len(eq_nc_lider) - len(eq_nc_comum_sesmt)

        # NOVA LÓGICA: total de reprovações distintas (sem repetir equipes em comum)
        eq_nc_geral = set(eq_nc_sesmt) | set(eq_nc_lider)  # união sem duplicatas
        eq_nc_geral_lista = sorted(eq_nc_geral)

        linha_geral = pd.DataFrame({
        "Grupo": ["GERAL"],
        "EQUIPES (n)": [len(eq_insp_geral)],
        "Equipes Inspecionadas": [resumir_com_tooltip(eq_insp_geral)],
        "REPROVAÇÕES (n)": [len(eq_nc_geral)],
        "Equipes Reprovadas": [resumir_com_tooltip(eq_nc_geral)],
        "EM COMUM COM NC(n)": [len(eq_nc_comum_geral)],
        "Equipes em Comum NC": [resumir_com_tooltip(eq_nc_comum_geral)],
        "SEM REPROVAÇÕES (n)": [len(eq_sem_nc_geral)],
        "Equipes Sem Reprovação": [resumir_com_tooltip(eq_sem_nc_geral)],
        "EQUIPES NÃO INSPECIONADAS (n)": [len(eq_nao_insp_geral)],
        "Equipes Não Inspecionadas": [resumir_com_tooltip(eq_nao_insp_geral)],
        "AMBOS SEM INSPEÇÃO (n)": [len(equipes_ambos_sem_inspecao)],
        "Equipes Ambos Sem Inspeção": [resumir_com_tooltip(equipes_ambos_sem_inspecao)]
    })
        ...
        "Grupo": ["GERAL"],
        "EQUIPES (n)": [len(eq_insp_geral)],
        "Equipes Inspecionadas": [formatar_lista_completa(eq_insp_geral)],
        "REPROVAÇÕES (n)": [len(eq_nc_geral)],
        "Equipes Reprovadas": [formatar_lista_completa(eq_nc_geral)],
        "EM COMUM (n)": [len(eq_nc_comum_geral)],
        "Equipes em Comum Reprov": [formatar_lista_completa(eq_nc_comum_geral)],
        "SEM REPROVAÇÕES (n)": [len(eq_sem_nc_geral)],
        "Equipes Sem Reprovação": [formatar_lista_completa(eq_sem_nc_geral)],
        "EQUIPES NÃO INSPECIONADAS (n)": [len(eq_nao_insp_geral)],
        "Equipes Não Inspecionadas": [formatar_lista_completa(eq_nao_insp_geral)],
        "AMBOS SEM NC (n)": [""],
        "AMBOS SEM INSPEÇÃO (n)": [len(equipes_ambos_sem_inspecao)],
        "Equipes Ambos Sem Inspeção": [formatar_lista_completa(equipes_ambos_sem_inspecao)]
        })


        # Criar dataframe para exportar com as listas completas (sem HTML nem tooltips)
        df_para_excel = pd.DataFrame({
            "Grupo": ["SESMT", "LIDERANÇA"],
            "EQUIPES (n)": [len(eq_insp_sesmt), len(eq_insp_lider)],
            "Equipes Inspecionadas": [formatar_lista_completa(eq_insp_sesmt), formatar_lista_completa(eq_insp_lider)],
            "REPROVAÇÕES (n)": [len(eq_nc_sesmt), len(eq_nc_lider)],
            "Equipes Reprovadas": [formatar_lista_completa(eq_nc_sesmt), formatar_lista_completa(eq_nc_lider)],
            "EM COMUM (n)": [len(eq_nc_comum_sesmt), len(eq_nc_comum_lider)],
            "Equipes em Comum Reprov": [formatar_lista_completa(eq_nc_comum_sesmt), formatar_lista_completa(eq_nc_comum_lider)],
            "SEM REPROVAÇÕES (n)": [len(eq_sem_nc_sesmt), len(eq_sem_nc_lider)],
            "Equipes Sem Reprovação": [formatar_lista_completa(eq_sem_nc_sesmt), formatar_lista_completa(eq_sem_nc_lider)],
            "EQUIPES NÃO INSPECIONADAS (n)": [len(eq_nao_insp_sesmt), len(eq_nao_insp_lider)],
            "Equipes Não Inspecionadas": [formatar_lista_completa(eq_nao_insp_sesmt), formatar_lista_completa(eq_nao_insp_lider)],
            "AMBOS SEM NC (n)": [len(equipes_ambos_sem_nc), len(equipes_ambos_sem_nc)],
            "AMBOS SEM INSPEÇÃO (n)": [len(equipes_ambos_sem_inspecao), len(equipes_ambos_sem_inspecao)],
            "Equipes Ambos Sem Inspeção": [formatar_lista_completa(equipes_ambos_sem_inspecao), formatar_lista_completa(equipes_ambos_sem_inspecao)],
        })

        # Adiciona a linha GERAL ao final do df_para_excel
    ...
    with EQUIPES_INSTRUTOR:
        ...
        df_eventos_apr = df_eventos_apr.merge(
            df_sem_pessoas[['idtb_turnos', 'nome_inspetor', 'num_operacional', 'unidade', 'nom_fant', 'funcao_geral']],
            on='idtb_turnos',
            how='left'
        ).drop_duplicates(subset=['idtb_turnos', 'nome_inspetor', 'num_operacional'])
        ...
        def render_blocos(df_base, funcoes, titulo, emoji, coluna):
            ...
            grupo = df_filtrado.groupby(['nome_inspetor', 'num_operacional'])['idtb_turnos_blitz_contatos'] \
            ...
            with coluna:
                ...
                for _, linha in grupo_total.iterrows():
                    ...
                    linhas = [f"Equipe {row['num_operacional']} – **{row['quantidade']}x**" for _, row in equipes.iterrows()]
        ...
        if not df_export.empty:
            df_export_grouped = df_export.groupby(['tipo_inspecao', 'nome_inspetor', 'num_operacional']) \
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                for grupo in ['SESMT', 'LIDERANÇA']:
                    ...
                    df_temp[['nome_inspetor', 'num_operacional', 'quantidade']].to_excel(
    ...
    with Horas_das_inspecao:    
        ...
        with col1:
            ...
            st.markdown(
                """
                <div style="text-align: center;">
                <h4 style="font-size:28px;"><strong>INSPEÇÕES POR HORA DO DIA</strong></h4>
                </div>
                """,
                unsafe_allow_html=True
            )
